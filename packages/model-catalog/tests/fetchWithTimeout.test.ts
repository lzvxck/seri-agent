...
describe("fetchWithTimeout", () => {
  ...
  // The regression this file exists to close: `fetchFn` resolving only means the response HEADERS
  // arrived, not that the whole response did. An earlier version returned the bare `Response` and
  // cleared the timer the moment `fetchFn` settled, so a caller's own LATER body read (outside
  // this function entirely) had no timeout guarding it at all — a response whose body stream never
  // closes hung forever past this function's own return. `read` now runs inside the same try the
  // timer lives in, so a hung body read still trips the deadline — mirroring how a real fetch()
  // Response's body reader is tied to the SAME AbortSignal the initial request used, which the
  // fake response body below reproduces explicitly.
