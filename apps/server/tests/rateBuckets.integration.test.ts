...
describe.skipIf(!process.env.SERI_TEST_SUPABASE_URL)(
  ...
  () => {
    ...
    test(
      "refill math: debiting to zero, waiting, then debiting again recovers roughly refill_rate * elapsed tokens",
      async () => {
    ...
    test(
      "concurrency: exactly `capacity` of N parallel debits against one fresh bucket succeed when refill_rate is 0",
      async () => {
        const supabase = testSupabase();
    ...
    test(
      "concurrency: exactly one of N parallel claim_concurrency_slot calls for the same user succeeds",
      async () => {
        const supabase = testSupabase();
