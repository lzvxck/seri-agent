...
// Free-tier request's isZeroPriceEntry check (quota.ts's own decidePreflight) for the rest of
// the instance's life over one transient models.dev failure. resetCatalogCache() clears it so
// the NEXT call retries instead; this is server-specific retry policy, not something loadCatalog
// itself should decide, since it has no way to know which callers' fallbacks are usable and
// which aren't.
