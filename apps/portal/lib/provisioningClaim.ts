 ...
 * a fresh customer's subscription on the very first call. Scoped to claimToken so a stale
 * claimant that wakes up after being reclaimed cannot delete a live claim it no longer owns.
 */
 ...
 * real error, and losing this only costs the delay the stale takeover exists to bound. Scoped
 * to claimToken for the same reason completeProvisioning is.
 */
