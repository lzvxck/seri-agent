...
// tui/commands.ts). Mirrors getApiKey's own env-then-config precedence and its deliberate `||`
// (an env var set to "" reads as unset, not as a valid-looking empty key) rather than re-deriving
// a second, possibly-drifting version of that lookup.
export function providerKeyState(provider: ModelProvider, configDir?: string): ProviderKeyState {
  const keyName = PROVIDER_API_KEY_NAMES[provider];
  const envValue = process.env[keyName];
  if (envValue) return { provider, keyName, source: "env", masked: maskValue(envValue) };
  const configValue = loadConfig(configDir)[keyName];
  if (configValue) return { provider, keyName, source: "config", masked: maskValue(configValue) };
  return { provider, keyName, source: "unset", masked: undefined };
}
