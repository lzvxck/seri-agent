...
function applyLoopEvent(state: TuiState, event: LoopEvent): TuiState {
  switch (event.type) {
    ...
    case "tool-result":
      return { ...pushLine(state, `✓ ${event.name} done`), status: "" };
    case "permission-denied":
      return { ...pushLine(state, `✗ ${event.name} blocked`), status: "" };
    case "tool-allowed":
      return {
    ...
    // Deliberately no-ops here, same as printEvent (cli/output.ts): the loop emits one `usage` per
    // completed model call, which would put a token count between every turn; `messages-updated`
    // carries the session's own business, not the screen's, and is applied via a `session-updated`
    // action instead.
    case "usage":
    case "messages-updated":
      return state;
    case "done":
      return { ...pushLine(state, `(done: ${event.reason})`), status: "" };
    case "error":
      return { ...pushLine(state, event.error), status: "" };
    default: {
