...
// (docs/specs/025-tui-opentui-migration/spec.md) even though the welcome-splash/guided-setup
// mounts this renderer doesn't yet cover (still separate ink mounts, wrapped by
// `runtime/legacyAltScreen.ts` until a later migration dispatch folds them into this same
// renderer) are unaffected by it — see that file's own comment for why both mechanisms coexist
// for now.
