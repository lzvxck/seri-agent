...
// and this repo's TUI code doesn't import `@opentui/core` yet (that's Phase 2's job) -- so this is
// the smallest thing that actually loads the native module rather than only the pure-JS surface.
// `createTestRenderer` is OpenTUI's own headless test harness (no real TTY required), and
